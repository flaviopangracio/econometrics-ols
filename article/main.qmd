...
Para aplicar as questões tratadas nas últimas seções, vamos estimar um modelo linear por MQO, utilizando a linguagem R e os dados da REGIC 2018. 
...
#| label: sum-pib
#| tbl-cap: Estatísticas descritivas do PIB per capita

summary_pib <- data.frame(as.list(summary(df_regic$pib_pc)))
names(summary_pib) <- names(summary(df_regic$pib_pc))
...
summary_ci <- data.frame(as.list(summary(df_regic$cige)))
names(summary_ci) <- names(summary(df_regic$cige))
